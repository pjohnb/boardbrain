            ...
            >
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  <h2 style={{ fontSize: '1.5rem', color: '#a78bfa' }}>
                  ...
                  <table style={{ 
                    ...
                    <thead>
                      <tr style={{ backgroundColor: '#334155' }}>
                        <th style={{ padding: '0.5rem', border: '1px solid #475569', textAlign: 'left', color: '#cbd5e1' }}>Type</th>
                        <th style={{ padding: '0.5rem', border: '1px solid #475569', textAlign: 'left', color: '#cbd5e1' }}>Card</th>
                        <th style={{ padding: '0.5rem', border: '1px solid #475569', textAlign: 'center', color: '#fbbf24' }}>Public</th>
                        {players.map((p, idx) => (
                          <th key={p.name} style={{ 
                            padding: '0.5rem', 
                            border: '1px solid #475569', 
                        ...
                        <th style={{ padding: '0.5rem', border: '1px solid #475569', textAlign: 'center', color: '#a78bfa' }}>
                          P(Solution)
                        </th>
                      </tr>
                    </thead>
                      ...
                      {['suspects', 'weapons', 'rooms'].map(category => (
                        CLUE_DATA[category].map((card, cardIdx) => {
                          ...
                          const getPlayerCell = (player, playerIdx) => {
                            // Check if dealt to this player
                            if (player.cards.includes(card)) {
                              if (playerIdx === currentPlayerIndex) {
                                return { text: 'D', color: '#60a5fa', bg: '#1e3a5f' }; // Your card
                              } else if (trainingMode) {
                                return { text: 'D', color: '#94a3b8', bg: '#374151' }; // Training: show all
                              } else if (cardState.shownTo?.includes(currentPlayer.name)) {
                                return { text: 'SHOWN', color: '#22c55e', bg: '#14532d' }; // Shown to you
                              } else {
                                return { text: '', color: '#64748b', bg: 'transparent' };
                              }
                            }
                            ...
                            constraints.forEach(constraint => {
                              if (constraint.cards.includes(card)) {
                                ...
                                if (constraint.suggester === player.name) {
                                  ...
                                  if (resp) {
                                    if (resp.action === 'pass') {
                                      cellContent = { text: '-', color: '#ef4444', bg: 'transparent' };
                                    } else if (resp.action === 'show') {
                                      if (resp.shownCard === card) {
                                        cellContent = { text: 'SHOWN', color: '#22c55e', bg: '#14532d' };
                                      } else if (resp.shownCard) {
                                        // They showed a different card
                                        cellContent = { text: '-', color: '#ef4444', bg: 'transparent' };
                                      } else {
                                        // They showed but we don't know which
                              ...
                              {players.map((p, playerIdx) => {
                                ...
                                return (
                                  <td key={p.name} style={{ 
                                    ...
                                    fontWeight: cell.text === 'D' || cell.text === 'SHOWN' ? '700' : 'normal',
                                    fontSize: cell.text === 'SHOWN' || cell.text === 'GUESS' ? '0.65rem' : '0.8rem'
                                  }}>
                                    {cell.text}
                                  </td>
                                );
                ...
                <div style={{ 
                  ...
                  <span><strong style={{ color: '#60a5fa' }}>D</strong> = Dealt (in hand)</span>
                  <span><strong style={{ color: '#fbbf24' }}>GUESS</strong> = Suggested this card</span>
                  <span><strong style={{ color: '#ef4444' }}>-</strong> = Passed (doesn't have)</span>
                  <span><strong style={{ color: '#22c55e' }}>SHOWN</strong> = Showed this card</span>
                  <span><strong style={{ color: '#94a3b8' }}>SHOWN</strong> = Showed (unknown which)</span>
                  <span><strong style={{ color: '#000', backgroundColor: '#1f2937', padding: '0 0.25rem' }}>X</strong> = 0% (not in solution)</span>
                  <span><strong style={{ color: '#a78bfa' }}>nn%</strong> = Probability in solution</span>
                </div>
          ...
          {selectedCard && (
            ...
            >
              ...
              >
                ...
                <div style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#0f172a', borderRadius: '0.375rem' }}>
                  <div style={{ fontSize: '1rem', fontWeight: '600', marginBottom: '0.5rem' }}>
                    Current Status
                  </div>
                  <div style={{ color: '#94a3b8' }}>
                    {cardStates[selectedCard]?.status === 'mine' ? (
                      <span style={{ color: '#60a5fa' }}>
                        🎴 Your Card
                      </span>
                    ) : cardStates[selectedCard]?.status === 'public' ? (
                      <span style={{ color: '#fbbf24' }}>
                        👁️ Public Card (Everyone Knows)
                      </span>
                    ) : cardStates[selectedCard]?.status === 'eliminated' ? (
                      <span style={{ color: '#ef4444' }}>
                        ❌ Eliminated - Held by {cardStates[selectedCard]?.holder}
                      </span>
                    ) : (
                      <span style={{ color: '#fbbf24' }}>
                        🎯 {cardStates[selectedCard]?.probability?.toFixed(0)}% likely in solution
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {constraints.filter(c => c.cards.includes(selectedCard)).length === 0 ? (
                    <p style={{ color: '#64748b' }}>This card hasn't been mentioned yet</p>
                  ) : (
                    constraints
                        ...
                        >
                          ...
                          <div style={{ fontSize: '0.875rem' }}>
                            {constraint.responses.map((resp, ridx) => (
                              ...
                              >
                                • {resp.player}: {resp.action === 'pass' ? 'Passed' : `Showed${resp.shownCard === selectedCard ? ` THIS CARD` : resp.shownCard ? ` (${resp.shownCard})` : ''}`}
                              </div>
                            ))}
                          </div>
                        </div>
